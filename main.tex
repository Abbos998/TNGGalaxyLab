...
\title[TNGGalaxyLab: Fourier pipeline for disc galaxies]{%
  \pipeline: A Reproducible Fourier Pipeline for Non-Axisymmetric
  Structure in Simulated Disc Galaxies}
...
\citep{Ohta+1990,Athanassoula+Misiriotis2002,Erwin2005}.
...
mean $\Amone^{\rm lit}$
(Eq.~\ref{eq:aperture_mean}), the area-weighted radial integral
$\Amone^{\rm area}$, and the Jog+2002 $\Sigma$-weighted integral
$\Amone^{\rm Jog}$ (Eq.~\ref{eq:jog_average}) — allowing a direct
...
suite \citep{Springel+2018,Pillepich+2018,Marinacci+2018,Naiman+2018},
run with the moving-mesh code \textsc{arepo} \citep{Springel2010}
and the TNG galaxy-formation model including kinetic AGN feedback
\citep{Weinberger+2017}.  First results on stellar and gaseous
disc structure are presented in \citet{Pillepich+2019} and
...
\citep{2019ComAC...6....2N,Pillepich+2019} at snapshot 99 ($z=0$) with
...
\citep[e.g.][]{Sheth+2010}.  The phase-coherence quality flag
...
be confused with the classical \citet{Toomre1964} $Q$ parameter,
with the effective multi-component stability parameter of
\citet{Romeo+Wiegert2011}, or with the
Efstathiou--Lake--Negroponte bar-stability parameter
\citep{Efstathiou+1982}, all of which measure distinct physical
quantities.
...
\citep[S$^{4}$G;][]{Sheth+2010} --- a volume-limited imaging
survey of 2352 nearby galaxies with uniform photometry
\citep{MunozMateos+2015} and morphological classification
\citep{Buta+2015} --- using the identical
inner radial aperture $[1.5, 2.5]\,\Rd$ as our pipeline.  Our
stellar-mass matching adopts the $3.6\,\mu$m mass-to-light
calibration of \citet{Meidt+2014} with the solar absolute
magnitude of \citet{Willmer2018}.  This
...
\citet{Frankel+2022} for TNG100 and provides quantitative
...
\citep[rank-based location shift;][]{Mann+Whitney1947}, the
Anderson--Darling $k$-sample test \citep[weighted sensitivity to
distribution tails;][]{Scholz+Stephens1987}, and Cliff's $\delta$
\citep[non-parametric effect size;][]{Cliff1993}.  For the TNG50-vs.-mass-matched
...
\citet{Romano+2006} scale.  For the TNG50-vs.-full-sample
...
timescales through phase mixing \citep{Sellwood2014}.  Second, in
...
\citep{Athanassoula+1985,Sellwood2014}.  Instead, external
...
bars once formed
\citep{Combes+Sanders1981,1993RPPh...56..173S,Debattista+Sellwood2000}
and with the observed weak dependence of bar frequency on
dynamical mass \citep{Erwin2018}.  Bar populations in
IllustrisTNG itself have been characterised by
\citet{RosasGuevara+2020}, and fast bars remain a point of
tension with $\Lambda$CDM haloes \citep{Fragkoudi+2021}.
...
\citep{TremaineWeinberg1984} for barred galaxies.
...
\citep{Sheth+2010}.  \pipeline\ development was supported by the
National University of Uzbekistan.

We acknowledge use of the open-source software \textsc{NumPy}
\citep{Harris+2020}, \textsc{SciPy} \citep{Virtanen+2020},
\textsc{Matplotlib} \citep{Hunter2007}, \textsc{Astropy}
\citep{AstropyCollaboration2022}, and \textsc{h5py}.
