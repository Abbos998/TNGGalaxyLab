...
(Aguerri et al.\ 2009).  Bootstrap resampling with $\Nboot = 200$
...
(Zenodo DOI: \href{https://doi.org/10.5281/zenodo.21363952}{10.5281/zenodo.21363952}).
\end{abstract}
...
\begin{enumerate}
  ...
  \item Bootstrap statistical uncertainties with $\Nboot = 200$
...
\begin{figure*}
  ...
  \begin{tikzpicture}[
    ...
    \node[decisionbox, below=of bar] (boot) {
      ...
      \footnotesize $N_{\rm boot}=200$ resamples \\[-1pt]
...
When the phase-coherent $m=2$ structure extends to the outermost
analysed radial bin, the reported bar length corresponds to the
analysis boundary and should therefore be interpreted as a lower
limit rather than a measured termination radius.  The radial
Fourier profiles are computed on a linear grid of $n_{\rm bins} = 40$
bins spanning $[0, r_{\rm out}]$ with $r_{\rm out} = 15\kpc$
(bin width $0.375\kpc$), so the outermost bin centre --- and hence
the maximum recoverable $L_{\rm bar}$ --- is at $14.8125\kpc$.
In the TNG50 catalogue this saturation occurs in $34.8\%$ of galaxies
(318/915), where the recovered $L_{\rm bar}$ equals the outermost
bin centre; these values are flagged in the machine-readable catalogue
as boundary-limited.  The saturated fraction is dominated by discs in
which a strong bar and its outer spiral extension share a common
phase, so that the coherence criterion is satisfied throughout the
adopted radial grid.  Statistical use of $L_{\rm bar}$ in this paper
is restricted to the detection dichotomy $f_{\rm coh} > 0.5$ vs.\
$f_{\rm coh} \le 0.5$; no distribution of bar-length values is
reported for astrophysical inference.
...
\citep[][§13.4]{Efron1995AnIT}, with $\Nboot = 200$
...
\begin{algorithm}[t]
  ...
  \For{$b = 1, \dots, N_{\rm boot}$}{
    ...
    Compute the aperture-averaged amplitude
       ...
       following Eq.~(\ref{eq:literature_average})\;
...
\section{Application to Illustris TNG50}
\label{sec:tng50}
...
exceed $\Amone > 0.2$; the bulk of the distribution is bounded
by $\Amone \lesssim 0.4$, with a single exceptional outlier at
$\Amone = 0.952 \pm 0.007$ discussed in
Section~\ref{sec:amplitude_outlier}.  Strongly lopsided disc
galaxies constitute a natural candidate set for individual case
studies of accretion-driven or halo-driven $m=1$ modes.

\begin{figure}
  \centering
  \includegraphics[width=\columnwidth]{figures/fig4_A1_distribution.pdf}
  \caption{Histogram of bootstrap-mean $\Amone$ across the
    915-galaxy TNG50 sample, displayed over $\Amone \in [0, 0.5]$
    for readability.  Dashed line: sample median (0.097); dotted
    line: sample mean ($0.112 \pm 0.070$).
    Orange: strongly lopsided ($\Amone > 0.1$, 431 galaxies,
    47.1\%); red: very strong ($\Amone > 0.2$, 86 galaxies,
    9.4\%).  One galaxy (subhalo 668581, $\Amone = 0.952$) lies
    outside the plotted range but is retained in all reported
    statistics; see Section~\ref{sec:amplitude_outlier}.}
  \label{fig:fig4_A1_distribution}
\end{figure}

\subsection{The single extreme outlier at $\Amone = 0.95$}
\label{sec:amplitude_outlier}

Of the 915 galaxies in the parent sample, exactly one --- TNG50-1
subhalo 668581 --- exhibits $\Amone$ well beyond the $\lesssim 0.4$
tail occupied by the remainder of the distribution.  The measurement
is not driven by shot noise: with a bootstrap mean of
$0.952 \pm 0.007$, the signal is detected at $\sim\!144\sigma$
against sampling variance from $N_{\rm boot} = 200$ resamples.  The
object is a compact dwarf disc
($M_\star = 5.4\times10^{9}\,\Msun$, $R_d = 1.73\kpc$,
$V_{\rm max} = 95\kms$) with a large disc-particle count
($N_{\rm disk} = 8.87\times10^{4}$) and coexisting strong $m=2$
mode ($\Amtwo = 0.30$, $f_{\rm coh} = 0.91$).  Several physical
scenarios --- an ongoing minor merger, a strongly displaced
dark-matter halo centre, or SUBFIND segmentation of an
interacting pair --- are all consistent with the amplitude
level, and none can be ruled out from the catalogue quantities
alone.  A dedicated follow-up (particle-level visual inspection
and merger-tree history) is deferred to future work.  We
emphasise that the exclusion of subhalo~668581 has no material
impact on any of the statistics reported in this paper: the
median, 16--84 percentile, and $47.1\%$ fraction above $\Amone > 0.1$
are identical to four significant figures whether the object is
retained or removed, and the sample mean changes only from $0.112$
to $0.111$.  We therefore keep the object in the parent sample but
flag it in the catalogue as a candidate for individual case study.
...
\begin{figure}
  ...
  \caption{Bootstrap-mean $\Amone$ vs.\ stellar mass, colour-coded
    ...
    Dotted line: $\Amone = 0.1$ threshold.  One galaxy
    (subhalo~668581, $\Amone = 0.952$;
    see Section~\ref{sec:amplitude_outlier}) lies outside the
    plotted range but is included in all statistics.}
...
Section~\ref{sec:dynamical}) demonstrates that dynamically
...
\url{https://github.com/Abbos998/TNGGalaxyLab}
(Zenodo DOI \href{https://doi.org/10.5281/zenodo.21363952}{10.5281/zenodo.21363952}).  The repository includes a
...
truncated excerpt showing the twenty most lopsided disc galaxies
sorted by bootstrap-mean $\Amone$ is given in
Table~\ref{tab:catalog_excerpt}.

\begin{table}
  \centering
  \caption{Primary columns of the TNG50 demonstration catalogue.  The
    machine-readable release contains 29 columns per galaxy in total;
    the ones listed below are the fields referenced in this paper.
    Additional columns include integral-variant amplitudes
    ($A_m^{\rm integral,area}$, $A_m^{\rm integral,Jog}$), the
    $m=2$ phase-scatter diagnostic, and bar-length bootstrap
    statistics.}
  ...
  \begin{tabular}{lll}
    ...
    \texttt{aperture\_R\_in\_kpc}  & kpc     & Inner aperture radius ($1.5\,R_d$) \\
    \texttt{aperture\_R\_out\_kpc} & kpc     & Outer aperture radius ($2.5\,R_d$) \\
...
\begin{table*}
  \centering
  \caption{Twenty most lopsided disc galaxies from the TNG50-1 sample,
    sorted by bootstrap-mean $\Amone$ in descending order.  Bootstrap
    uncertainties are quoted at $1\sigma$ from $N_{\rm boot}=200$
    resamples.  Following Section~\ref{sec:bar_length}, the
    phase-coherent bar length $L_{\rm bar}$ is used only through the
    detection dichotomy $f_{\rm coh} > 0.5$ vs.\ $f_{\rm coh} \le 0.5$
    and is not tabulated here; the full machine-readable catalogue
    reports it as a distinct column
    (\texttt{bar\_length\_kpc}, see Table~\ref{tab:catalog_columns}).
    The complete 915-galaxy catalogue is available via the code
    repository (Zenodo DOI:~10.5281/zenodo.21363952).}
  ...
  \begin{tabular}{rccccc}
    ...
    $f_{\rm coh}$ \\
    \midrule
    668581  &  $5.37\!\times\!10^{9}$  &  1.73  &  $0.952 \pm 0.007$  &  $0.302 \pm 0.008$  &  0.91  \\
    661532  &  $3.24\!\times\!10^{9}$  &  2.04  &  $0.499 \pm 0.022$  &  $0.803 \pm 0.015$  &  0.72  \\
    646803  &  $3.97\!\times\!10^{9}$  &  1.57  &  $0.455 \pm 0.011$  &  $0.105 \pm 0.012$  &  0.55  \\
    558067  &  $2.03\!\times\!10^{10}$  &  2.60  &  $0.448 \pm 0.007$  &  $0.502 \pm 0.007$  &  0.81  \\
    650623  &  $4.87\!\times\!10^{9}$  &  2.67  &  $0.391 \pm 0.011$  &  $0.111 \pm 0.011$  &  0.44  \\
    620172  &  $4.29\!\times\!10^{9}$  &  2.58  &  $0.380 \pm 0.016$  &  $0.189 \pm 0.020$  &  0.89  \\
    655323  &  $6.02\!\times\!10^{9}$  &  2.42  &  $0.377 \pm 0.009$  &  $0.194 \pm 0.009$  &  0.73  \\
    645391  &  $5.79\!\times\!10^{9}$  &  3.82  &  $0.370 \pm 0.010$  &  $0.237 \pm 0.012$  &  0.00  \\
    657568  &  $4.36\!\times\!10^{9}$  &  2.70  &  $0.368 \pm 0.012$  &  $0.229 \pm 0.010$  &  0.09  \\
    647768  &  $4.52\!\times\!10^{9}$  &  2.64  &  $0.367 \pm 0.013$  &  $0.336 \pm 0.013$  &  0.73  \\
    663812  &  $6.39\!\times\!10^{9}$  &  2.01  &  $0.350 \pm 0.011$  &  $0.073 \pm 0.011$  &  0.44  \\
    594246  &  $6.38\!\times\!10^{9}$  &  1.84  &  $0.330 \pm 0.009$  &  $0.220 \pm 0.009$  &  0.31  \\
    663308  &  $5.06\!\times\!10^{9}$  &  1.82  &  $0.329 \pm 0.010$  &  $0.084 \pm 0.009$  &  0.18  \\
    661449  &  $5.21\!\times\!10^{9}$  &  4.18  &  $0.327 \pm 0.009$  &  $0.214 \pm 0.010$  &  0.29  \\
    578834  &  $1.64\!\times\!10^{10}$  &  3.75  &  $0.316 \pm 0.007$  &  $0.212 \pm 0.007$  &  0.82  \\
    600743  &  $9.61\!\times\!10^{9}$  &  2.59  &  $0.305 \pm 0.016$  &  $0.203 \pm 0.014$  &  0.93  \\
    663231  &  $3.28\!\times\!10^{9}$  &  2.47  &  $0.300 \pm 0.016$  &  $0.098 \pm 0.015$  &  0.88  \\
    715911  &  $5.04\!\times\!10^{9}$  &  1.35  &  $0.298 \pm 0.010$  &  $0.082 \pm 0.008$  &  0.38  \\
    670767  &  $4.13\!\times\!10^{9}$  &  1.90  &  $0.295 \pm 0.014$  &  $0.065 \pm 0.012$  &  0.09  \\
    638742  &  $7.03\!\times\!10^{9}$  &  2.22  &  $0.290 \pm 0.008$  &  $0.067 \pm 0.009$  &  0.14  \\
    \bottomrule
  \end{tabular}
\end{table*}


\section{Independent reproducibility audit}
\label{app:reproducibility_audit}

Table~\ref{tab:reproducibility_audit} lists the principal statistics
quoted in the main text alongside the values recomputed directly
from the released catalogue (\texttt{catalog.csv}).  All
computations were performed with an independent Python
re-implementation using only the standard library
(\texttt{csv}, \texttt{statistics}, \texttt{math}), without
recourse to any part of the \pipeline\ codebase.  Spearman
rank correlations were evaluated with a small custom function
identical in behaviour to \texttt{scipy.stats.spearmanr}.

\begin{table*}
  \centering
  \caption{Comparison of manuscript statistics with values
    independently recomputed from the released catalogue.
    Bootstrap ranks, correlations, and threshold-based sample counts
    reproduce the tabulated values to the precision printed in the
    main text.  Small differences in the fourth decimal place
    reflect rounding of the printed quantities; no discrepancy
    exceeds the corresponding bootstrap uncertainty.}
  \label{tab:reproducibility_audit}
  \begin{tabular}{lccc}
    \toprule
    Statistic & Value in paper & Recomputed & Difference \\
    \midrule
    Median $\Amone$                                      &     0.0970 &     0.0974 &     0.0004 \\
    Mean $\Amone$                                        &     0.1120 &     0.1117 &     0.0003 \\
    $1\sigma$ of $\Amone$ distribution                   &     0.0700 &     0.0702 &     0.0002 \\
    $16$th percentile of $\Amone$                        &     0.0560 &     0.0560 &     0.0000 \\
    $84$th percentile of $\Amone$                        &     0.1610 &     0.1606 &     0.0004 \\
    \midrule
    Galaxies with $\Amone > 0.1$                         &        431 &        431 &          0 \\
    Galaxies with $\Amone > 0.2$                         &         86 &         86 &          0 \\
    Galaxies with $f_{\rm coh} > 0.5$                    &        431 &        431 &          0 \\
    Overlap: $\Amone > 0.1$ \emph{and} $f_{\rm coh} > 0.5$ &        185 &        185 &          0 \\
    \midrule
    $\rho_{\rm Spearman}(Q_{\rm dyn}, \Amone)$           &     +0.350 &     +0.350 &      0.000 \\
    $\rho_{\rm Spearman}(V_{\rm max}, \Amone)$           &     -0.390 &     -0.387 &      0.003 \\
    $\rho_{\rm Spearman}(V_{\rm max}, \Amtwo)$           &     -0.050 &     -0.054 &      0.004 \\
    Median $Q_{\rm dyn}$                                 &       2.60 &       2.65 &       0.05 \\
    \bottomrule
  \end{tabular}
\end{table*}

The auditor's script is included in the code repository as
\texttt{scripts/reproducibility\_audit.py} and can be re-executed
by any reader with a copy of \texttt{catalog.csv} and a standard
Python 3 interpreter.  We invite the community to extend this
audit table with any further statistic of interest.
